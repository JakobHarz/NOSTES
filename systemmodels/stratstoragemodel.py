...
class Strat_storage_model(SystemModel):
    def __init__(self, s_n, g_n, distance, data: Data):
        ...
        g_vec = ca.vertcat(const_bat1, const_bat2, const_hp, const_mdot_hp,const_Grid)
    ...
    def compute_ground_properties(self, g_n, distance):
        ...
        self.d = [0] + [self.distance / 2] + [self.distance / 2 + self.distance * (i+1) for i in range(self.g_n-1)] + [self.distance *self.g_n]
