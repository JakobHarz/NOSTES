...
class AveragedStratStorageModel(SystemModel):
    def __init__(self, s_n, g_n, distance, data: Data):
        ...
        const_bat1 = self.u[1] - self.C_bat
        const_bat2 = self.u[2] - self.C_bat
        const_hp = self.u[0] - self.C_hp  # W
        g_vec = ca.vertcat(const_bat1, const_bat2, const_hp, const_Grid)
        self.g_fine = ca.Function('g', [self.x_bat, self.u, self.p_fix, self.p_data], [g_vec], ['x', 'u', 'p_fix', 'p_data'],
                                  ['g'])
        self.lbg_fine = ca.vertcat(-ca.inf * ca.DM.ones((g_vec.shape[0] - 1, 1)), 0)
        self.ubg_fine = ca.DM.zeros(g_vec.shape)
        self.ng_fine = g_vec.shape[0]
    ...
    def compute_fixed_cost(self, p_fix, annuity, n_year):
        ...
        self.C_hp = 7e6 * self.p_fix[1]  # W
        CAPEX = I_hp * self.C_hp + I_s * self.V_s + I_pv * self.C_pv + I_wind * self.C_wind + I_bat * self.C_bat
        OPEX = 0.01 * CAPEX  # 1% of CAPEX
