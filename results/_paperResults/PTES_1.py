...
class ThermalModel:
    ...
    def formulate_state_space_model(self):
        A = []
        A.append([-1/self.C_sto * ((1/(self.R_sto + self.R_ground[0])) + 1/self.R_top)] + [1/self.C_sto * (1/(self.R_sto + self.R_ground[0]))] + [0]*(self.iter-1))
        A.append([1/self.C_ground[0] * (1/(self.R_sto + self.R_ground[0])), -1/self.C_ground[0] * ((1/self.R_ground[1]) + (1/(self.R_sto + self.R_ground[0]))), 1/self.C_ground[0]*(1/self.R_ground[1])] + [0]*(self.iter-2))
        for i in range(1, self.iter-1):
            A.append([0]*i + [1/self.C_ground[i]*(1/self.R_ground[i]), -1/self.C_ground[i]*((1/self.R_ground[i])+ (1/self.R_ground[i+1])), 1/self.C_ground[i]*(1/self.R_ground[i+1])] + [0]*(self.iter-2-i))
        A.append([0]*(self.iter-1) + [1/self.C_ground[self.iter-1]*(1/self.R_ground[self.iter-1]), -1/self.C_ground[self.iter-1]*((1/self.R_ground[self.iter-1])+(1/self.R_ground[self.iter]))])
        self.A = ca.SX(A)

        B = []
        B.append([1/self.C_sto, 0])
        for _ in range(self.iter):
            B.append([0, 0])
        self.B = ca.SX(B)
        D = []
        for _ in range(self.iter):
            D.append([0, 0, 0, 0, 1/self.C_sto * 1/self.R_top])
        D.append([1/(self.C_ground[self.iter-1]*self.R_ground[self.iter]), 0, 0, 0, 0])
        self.D = ca.SX(D)
        self.Tdot = ca.vertcat(self.A@self.state + self.B@self.con + self.D@self.par)  # [K/s]
...
model = ThermalModel(153.3, 73.2, iter=2, distance = 0.5, U_wall = 80) # Dronninglund
